...
# RTSP stream URLs per camera name (lowercase, underscored)
CAMERA_STREAMS = {
    "parking_lot": "rtsp://fuGk55rSwiDtVigRfxPRkVlyiJPGmDZW:ZqniBPd7ILatymFgJcK9W@test.rtsp.stream/people",
}

# Default stream used by all cameras not listed above
...
def generate(camera_name=None):
    """
    Generate MJPEG frames from an RTSP stream.

    Args:
        camera_name: The camera_name value from the cameras table (e.g. "Parking Lot").
                     Used to look up a camera-specific RTSP URL.  Falls back to the
                     default stream when None or not found in CAMERA_STREAMS.
    """
    # Normalise the camera name the same way the frontend does when building
    # the stream URL: lowercase + spaces → underscores.
    key = camera_name.lower().replace(" ", "_") if camera_name else None
    rtsp_url = CAMERA_STREAMS.get(key, DEFAULT_RTSP_URL)
